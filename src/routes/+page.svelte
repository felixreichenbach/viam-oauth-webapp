<script lang="ts">
  ...
  async function constructFusionAuthURL() {
    ...
    const port = 5173;
    return `${fusionAuthBaseURL}/oauth2/authorize?client_id=${clientId}&response_type=code&redirect_uri=http://localhost:${port}/auth&scope=offline_access&state=${stateValue}&code_challenge=${challenge}&code_challenge_method=S256`;
  }
